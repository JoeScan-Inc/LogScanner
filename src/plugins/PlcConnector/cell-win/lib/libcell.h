...
#ifdef __cplusplus
//Tells the compiler to use C symbol naming conventions
extern "C" {
#endif //__cplusplus
...
#define CELL_DFLT_TIMEOUT 2000  /* changed timeout  */
...
#pragma pack(1)
	...
	typedef struct
	{
        ...
        unsigned short size;//Ths size of each element
		unsigned short alias_size;
		unsigned long arraysize1;//first dimension
		unsigned long arraysize2;//second dimension
		unsigned long arraysize3;//third dimension
        unsigned short datalen;//The number of bytes
		long dirty;
		byte *data;
...
#define CELLERROR(num,msg) { cell_line=__LINE__; strcpy(cell_file,__FILE__); cell_errno = num; strcpy(cell_err_msg,msg); }
	...
	__EXPORT extern int js_connect(char *hostname, int device_number_cpu);
	__EXPORT extern void js_disconnect();
	__EXPORT extern int js_read_tag_8(char *tagname, char *value);
	__EXPORT extern int js_read_tag_16(char *tagname, short *value);
	__EXPORT extern int js_read_tag_32(char *tagname, int *value);
	
	__EXPORT extern int js_write_tag_32(char *tagname, int value);
	__EXPORT extern int js_write_tag_16(char *tagname, short value);
	__EXPORT extern int js_write_tag_8(char *tagname, char value);

	__EXPORT extern int js_write_tag_array_32(char *tagname, int* values, int count);

	__EXPORT extern void setDPrintCallback(char *);
	__EXPORT extern void setDebugLevel(int level);
